...
async function loadingAllTypesPokemon() {
  ...
  try {
    ...
    let pokemonLimitPage =
      POKEMON_API_ENDPOINT + "?limit=" + "&offset=" + changePokemonPerPage;
...
async function listByType() {
  ...
  try {
    ...
    let limitPerPage =
      listPokemonTypes.pokemon.length > 100
        ? 100
        : listPokemonTypes.pokemon.length; // Limita a 100
...
function pokemonGrid() {
  ...
  for (let i = 0; i < pokemonFilteredList.length; i++) {
    ...
    var html =
      '<div class="c" onclick="showDetails(' + filteredListElement.id + ')">';
    html =
      html +
      '<img src="' +
      filteredListElement.sprites.front_default +
      '" class="i" alt="' +
      filteredListElement.name +
      '">';
    html =
      html +
      '<h5 class="text-center">#' +
      filteredListElement.id +
      " " +
      filteredListElement.name.charAt(0).toUpperCase() +
      filteredListElement.name.slice(1) +
      "</h5>";
...
async function showDetails(id) {
  try {
    ...
    for (
      let index = 0;
      index < pokemonDetailsList.flavor_text_entries.length;
      index++
    ) {
      if (
        pokemonDetailsList.flavor_text_entries[index].language.name === "en"
      ) {
    ...
    document.getElementById("modalTitle").textContent =
      "#" +
      pokemonDetailsRote.id +
      " " +
      pokemonDetailsRote.name.charAt(0).toUpperCase() +
      pokemonDetailsRote.name.slice(1);

    let placeHolder = '<div class="row"><div class="col-md-6">';
    placeHolder += '<div class="sprite-container">';
    placeHolder +=
      '<div><img src="' +
      pokemonDetailsRote.sprites.front_default +
      '" alt="front"><p class="text-center">Normal</p></div>';
    placeHolder +=
      '<div><img src="' +
      pokemonDetailsRote.sprites.front_shiny +
    ...
    for (let index = 0; index < pokemonDetailsRote.types.length; index++) {
      placeHolder +=
        '<span class="badge type-' +
        pokemonDetailsRote.types[index].type.name +
        '">' +
        pokemonDetailsRote.types[index].type.name +
        "</span> ";
    }
    ...
    placeHolder +=
      "<p><strong>Altura:</strong> " + pokemonDetailsRote.height / 10 + "m</p>";
    placeHolder +=
      "<p><strong>Peso:</strong> " + pokemonDetailsRote.weight / 10 + " kg</p>";
